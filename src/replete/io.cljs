  ...
  (let [file-uri (Uri. (.getPath jar-uri))]
    (if (file-uri? file-uri)
      (let [[file-path resource] (string/split (.getPath file-uri) #"!/")
            [content error-msg] (js/REPLETE_LOAD_FROM_JAR file-path resource)]
        (if-not (nil? content)
          (#'replete.core/make-string-reader content)
...
(defn- make-bundled-uri-reader
  ...
  (let [path    (.getPath bundle-uri)
        content (first (js/REPLETE_LOAD path))]
    (#'replete.core/make-string-reader content)))

(defn- make-http-uri-reader
  [uri opts]
  (#'replete.core/make-string-reader (:body (http/get (str uri) {}))))
...
(extend-protocol IOFactory
  ...
  (make-reader [file opts]
    (let [file-descriptor (js/REPLETE_FILE_READER_OPEN (:path file) (:encoding opts))]
      ...
      (#'replete.core/->Reader
        (fn []
          (if (contains? @open-file-reader-descriptors file-descriptor)
            (let [[result err] (js/REPLETE_FILE_READER_READ file-descriptor)]
              (if err
                (throw (js/Error. err)))
              result)
        ...
        (fn []
          (when (contains? @open-file-reader-descriptors file-descriptor)
            ...
            (js/REPLETE_FILE_READER_CLOSE file-descriptor)))
        (atom nil)
        (atom 0))))
  (make-writer [file opts]
    (let [file-descriptor (js/REPLETE_FILE_WRITER_OPEN (:path file) (boolean (:append opts)) (:encoding opts))]
      (check-file-descriptor file-descriptor file opts)
      (swap! open-file-writer-descriptors conj file-descriptor)
      (#'replete.core/->Writer
        (fn [s]
          (if (contains? @open-file-writer-descriptors file-descriptor)
            (if-let [err (js/REPLETE_FILE_WRITER_WRITE file-descriptor s)]
              (throw (js/Error. err)))
            (throw (js/Error. "File closed.")))
          nil)
        (fn []
          (if (contains? @open-file-writer-descriptors file-descriptor)
            (if-let [err (js/REPLETE_FILE_WRITER_FLUSH file-descriptor)]
              (throw (js/Error. err)))
            (throw (js/Error. "File closed.")))
          nil)
        (fn []
          (when (contains? @open-file-writer-descriptors file-descriptor)
            ...
            (js/REPLETE_FILE_WRITER_CLOSE file-descriptor))))))
  (make-input-stream [file opts]
    (let [file-descriptor (js/REPLETE_FILE_INPUT_STREAM_OPEN (:path file))]
      (check-file-descriptor file-descriptor file opts)
      (swap! open-file-input-stream-descriptors conj file-descriptor)
      (#'replete.core/->InputStream
        (fn []
          (if (contains? @open-file-input-stream-descriptors file-descriptor)
            (some-> (js/REPLETE_FILE_INPUT_STREAM_READ file-descriptor) vec)
            (throw (js/Error. "File closed."))))
        (fn []
          (when (contains? @open-file-input-stream-descriptors file-descriptor)
            ...
            (js/REPLETE_FILE_INPUT_STREAM_CLOSE file-descriptor))))))
  (make-output-stream [file opts]
    (let [file-descriptor (js/REPLETE_FILE_OUTPUT_STREAM_OPEN (:path file) (boolean (:append opts)))]
      (check-file-descriptor file-descriptor file opts)
      (swap! open-file-output-stream-descriptors conj file-descriptor)
      (#'replete.core/->OutputStream
        (fn [byte-array]
          (if (contains? @open-file-output-stream-descriptors file-descriptor)
            (js/REPLETE_FILE_OUTPUT_STREAM_WRITE file-descriptor (into-array byte-array))
            (throw (js/Error. "File closed."))))
        (fn []
          (if (contains? @open-file-output-stream-descriptors file-descriptor)
            (js/REPLETE_FILE_OUTPUT_STREAM_FLUSH file-descriptor)
            (throw (js/Error. "File closed."))))
        (fn []
          (when (contains? @open-file-output-stream-descriptors file-descriptor)
            ...
            (js/REPLETE_FILE_OUTPUT_STREAM_CLOSE file-descriptor))))))
  ...
  default
  (make-reader [x _]
    (if (satisfies? replete.core/IReader x)
      x
  ...
  (make-input-stream [x _]
    (if (satisfies? replete.core/IInputStream x)
      x
      (throw (ex-info (str "Can't make an input stream from " x) {}))))
  (make-output-stream [x _]
    (if (satisfies? replete.core/IOutputStream x)
      x
...
(defn delete-file
  "Delete file f."
  [f]
  (js/REPLETE_DELETE (:path (as-file f))))
...
(defn ^boolean directory?
  ...
  [dir]
  (js/REPLETE_IS_DIRECTORY (:path (as-file dir))))
...
(defn list-files
  ...
  [dir]
  (when (directory? dir)
    (map as-file (js/REPLETE_LIST_FILES (:path (as-file dir))))))
...
(defn resource
  ...
  [n]
  (when-some [[_ _ loaded-path loaded-type loaded-location] (js/REPLETE_LOAD n)]
    (case loaded-type
...
(defn- get-parent-file [file]
  (let [path (:path file)]
    (let [ndx (.lastIndexOf path "/")]
      (if (< ndx 1)
        (if (> (count path) 1)
          (replete.io/file path-separator)
          nil)
        (replete.io/file (subs path 0 ndx))))))
...
(defmethod do-copy [@#'replete.core/InputStream @#'replete.core/OutputStream]
  [input output opts]
  (loop []
    (when-some [byte-array (replete.core/-read-bytes input)]
      (do
        (replete.core/-write-bytes output byte-array)
        (recur)))))

(defmethod do-copy [@#'replete.core/InputStream @#'replete.core/Writer]
  [input output opts]
  (let [bytes      (->> (repeatedly #(replete.core/-read-bytes input))
                        (take-while some?)
                        (map vec)
                        (apply concat))
...
(defmethod do-copy [@#'replete.core/InputStream File]
  [input output opts]
  (with-open [out (output-stream output)]
             (do-copy input out nil)))

(defmethod do-copy [@#'replete.core/Reader @#'replete.core/OutputStream]
  [input output opts]
  (do-copy (replete.core/slurp input) output))

(defmethod do-copy [@#'replete.core/Reader @#'replete.core/Writer]
  [input output opts]
  (loop []
    (when-some [s (replete.core/-read input)]
      (do
        (-write output s)
        (recur)))))

(defmethod do-copy [@#'replete.core/Reader File]
  [input output opts]
  (with-open [out (writer output)]
             (do-copy input out nil)))

(defmethod do-copy [File @#'replete.core/OutputStream]
  [input output opts]
  (with-open [in (input-stream input)]
             (do-copy in output)))

(defmethod do-copy [File @#'replete.core/Writer]
  [input output opts]
  (with-open [in (reader input)]
             (do-copy in output nil)))

(defmethod do-copy [File File] [input output opts]
  (js/REPLETE_COPY (:path input) (:path output)))

(defmethod do-copy [js/String @#'replete.core/OutputStream]
  [input output opts]
  (let [str->utf8  (comp js/unescape js/encodeURIComponent)
        str->chars (fn [s] (map #(.charCodeAt %) s))]
    (replete.core/-write-bytes output (-> input str->utf8 str->chars to-array))))

(defmethod do-copy [js/String @#'replete.core/Writer]
  [input output opts]
  (do-copy (#'replete.core/make-string-reader input) output nil))

(defmethod do-copy [js/String
                    File]
  [input output opts]
  (with-open [out (writer output)]
             (do-copy (#'replete.core/make-string-reader input) out)))

(defn copy
  "Copies input to output. Returns nil or throws an exception.

  Input may be an IInputStream or IReader created using replete.io, File, or
  string.

  Output may be an IOutputStream or IWriter created using replete.io, or File.
...
(def ^:deprecated read-line replete.core/read-line)
(def ^:deprecated slurp replete.core/slurp)
(def ^:deprecated spit replete.core/spit)

(set! replete.core/*reader-fn* reader)
(set! replete.core/*writer-fn* writer)
(set! replete.core/*as-file-fn* as-file)
(set! replete.core/*file?-fn* file?)
